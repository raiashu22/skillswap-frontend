...
export default function Dashboard() {
  ...
  async function handleAction(action, request) {
    ...
    try {
      if (action === "accept") {
        await api.acceptRequest(request.id, token);
      }

      if (action === "decline") {
        await api.declineRequest(request.id, token);
      }

      if (action === "complete") {
        await api.completeRequest(request.id, token);
      }
    ...
    <div className="page">
      <div className="page-header">
        ...
        <p className="page-subtitle">
          Manage requests you've sent and received below.
        </p>
      </div>
      ...
      <div className="dash-stats">
        ...
        <div className="card dash-stat-card">
          <div
            ...
            style={{
              background:
                "linear-gradient(135deg, var(--purple-300), var(--purple-500))",
            }}
          >
            <Handshake size={18} />
          </div>
      ...
      {loading ? (
        ...
        <div className="empty-state">
          ...
          <p>
            Head to Browse skills to send your first
            request.
          </p>
        </div>
      ) : (
        <div className="request-list">
          {filtered.map((r, idx) => {
            ...
            return (
              <div
                key={r.id}
                className="card request-row stagger-item"
                style={{
              ...
              >
                <div className="request-row-info">
                  ...
                  <span className="request-row-meta">
                    {isProvider
                      ? `Requested by ${otherPerson}`
                      : `Providing: ${otherPerson}`}{" "}
                    · {r.credits} credit
                    {r.credits > 1 ? "s" : ""}
      ...
      {chattingWith && (
        <ChatPanel
          ...
          otherPersonName={
            chattingWith.providerId === user.id
              ? chattingWith.requester.name
              : chattingWith.provider.name
          }
